...
class LightningWrapper(L.LightningModule):
    ...
    def training_step(self, batch):
        ...
        mixed_prototypes = labels[:, :, None] * self.centroids.T[None, :, :]
        mixed_prototypes = F.normalize(mixed_prototypes.sum(dim=1), p=2, dim=1)

        embeddings = F.normalize(self.model(x), p=2, dim=1)

        cosines = (embeddings * mixed_prototypes).sum(dim=1)

        loss = ((1 - cosines) ** 2).sum()
